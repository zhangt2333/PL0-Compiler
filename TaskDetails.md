...
*   对于异常处理，专门设计一个异常类
*   TODO: 去掉异常处理器的 if，采用 assert 的思想重构异常处理器
...
*   重构 Code 类
    *   增加初始化值
    *   增加构造方法 `l`、`l f`、`l f a`，其中 `l f` 构造方法适用于 OPR 这种没有 level 语义的指令

设计一个符号表管理器：

*   管理符号表的生成、符号表插入符号、查询符号
*   使用 unordered_map 作为符号表数据结构
*   使用 stack 作为新增层次的符号表管理
*   使用 vector 作为符号表存储序列